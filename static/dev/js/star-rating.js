...
(function ($) {
    ...
    Rating.prototype = {
        ...
        initSlider: function (options) {
            ...
            self.initialValue = self.$element.val()/2;
        ...
        renderCaption: function () {
            var self = this, val = self.$element.val()/2;
            if (!self.showCaption) {
                return '';
            }
        ...
        setStars: function (pos) {
            var self = this, min = self.min, max = self.max, step = self.step,
                val = arguments.length ? self.getValueFromPosition(pos) : (isEmpty(self.$element.val()) ? 0 : self.$element.val()/2),
                width = 0, maxWidth = self.$rating.width(), caption = self.fetchCaption(val);
            width = (val - min) / max * 100;
            if (self.rtl) {
                width = 100 - width;
            }
            self.$element.val(val*2);
            width += '%';
        ...
        reset: function () {
            var self = this;
            self.$element.val(self.initialValue*2);
            self.setStars();
